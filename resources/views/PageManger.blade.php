<x-app-layout>
    <div class="mx-auto h-screen shadow-xl sm:rounded-lg" style="background:#02194fe0; border-radius: 16px; padding: 20px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1);">
        ...
        @if(session('success'))
            <div class="bg-green-500 text-white p-3 rounded-md mb-4 flex justify-between items-center">
                <span>{{ session('success') }}</span>
                <button onclick="this.parentElement.style.display='none'" class="text-white text-xl">&times;</button>
            </div>
        @endif
        @if(session('error'))
            <div class="bg-red-500 text-white p-3 rounded-md mb-4 flex justify-between items-center">
        ...
        <div class="pt-4 pb-5">
            <table class="table w-full table-auto text-white border-collapse">
                <thead>
                    <tr>
                        <th class="text-left align-middle p-4 border-b"><input type="checkbox" style="transform: scale(1.2);" /></th>
                ...
                <tbody>
                    ...
                    @else
                        @foreach($pages as $page)
                            <tr>
                                <td class="text-left align-middle p-4 border-b"><input type="checkbox" style="transform: scale(1.2);" /></td>
                                <td class="p-4 border-b">{{ $page->page_id }}</td>
                                <td class="p-4 border-b">{{ $page->page_slug }}</td>
                                <td class="p-4 border-b"><a href="{{ route('page_demo',$page->page_url) }}">{{ $page->page_url }}</a></td>
                                ...
                                <td class="p-4 border-b text-center">
                                    <a href="{{ route('page_demo',$page->page_url) }}" class="mx-1 px-3 py-1 bg-green-400 rounded-sm">View</a>
                                    <a href="{{ 
                                        route(
                                        'page_manager.modify', 
                                        $page->page_slug).
                                    ...
                                    <a href="{{ route('page.destroy', $page->page_id) }}" class="px-3 mx-2 py-1 bg-red-400 rounded-sm">DEL</a>
                                </td>
                            </tr>
                        @endforeach
                    @endif
